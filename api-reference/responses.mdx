---
title: "POST /v1/responses — OpenAI Responses Endpoint"
sidebarTitle: "Responses"
description: "POST /v1/responses — OpenAI Responses-style endpoint. Accepts string or array input, preserves multimodal content blocks, and emits structured SSE events when streaming."
---

The Responses endpoint is a compatibility layer that normalises input in the OpenAI Responses API format and routes it internally to Chat Completions. It accepts either a plain string or a structured array of message objects as `input`, making it easy to migrate existing Responses API integrations to Piramyd with no other code changes. Unlike Chat Completions, the streaming format uses named SSE events rather than generic `chat.completion.chunk` objects, and multimodal content blocks — including `input_image` — are preserved through the pipeline rather than flattened to text.
